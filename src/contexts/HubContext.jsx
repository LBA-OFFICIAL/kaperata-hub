...
export const HubProvider = ({ children, profile }) => {
  ...
  const notifications = useMemo(() => {
    const hasNew = (items, pageKey) => { 
      ...
      return lastVisit ? items.some(i => (i.createdAt?.toDate ? i.createdAt.toDate() : new Date(i.createdAt || 0)) > new Date(lastVisit)) : true;
    };
    return { 
      events: hasNew(events, 'events'), 
      announcements: hasNew(announcements, 'announcements'), 
      suggestions: hasNew(suggestions, 'suggestions'), 
      committee_hunt: isSuperAdmin ? committeeApps.some(a => a.status === 'pending') : userApplications.some(a => a.statusUpdatedAt?.toDate() > new Date(lastVisited['committee_hunt'] || 0)), 
