...
export const getEventDay = (dateInput) => { // --- LBA BRANDING & CONFIGURATION ---
...
export const getDirectLink = (url) => { 
  if (!url || typeof url !== 'string') return ""; 
  // Improved Robust Link Engine: Uses googleusercontent to bypass CORS
  if (url.includes('drive.google.com')) { 
    const idMatch = url.match(/[-\w]{25,}/); 
    if (idMatch) return `https://lh3.googleusercontent.com/d/$${idMatch[0]}=s1000`; 
  } 
  return url; 
};
...
export const getMemberIdMeta = () => { 
  ...
  // Current logic for 2026: 
  // If before August, SY is 2526. If August or later, SY is 2627.
  let startYear = month >= 8 ? year : year - 1;
  let endYear = startYear + 1;
  
  const sy = `${String(startYear).slice(-2)}${String(endYear).slice(-2)}`;
  
  // Semester 1: Aug-Dec, Semester 2: Jan-July
...
export const getDailyCashPasskey = () => { 
  ...
  // Example: KBA-1-326 (for March 1, 2026)
  return `KBA-${d}-${m}${y}`; 
};
